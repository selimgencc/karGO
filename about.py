###########################################################################################
###                        CODE:       WRITTEN BY: ANJAL.P AUGUST 11 2020               ###
###                        PROJECT:    PELLIS Z1                                        ###
###                        PURPOSE:    WINDOWS/LINUX/MAC OS FLAT MODERN UI              ###
###                                    BASED ON QT DESIGNER, PySide2                    ###
###                        USE CASE:   TEMPLATE FOR SOFTWARES                           ###
###                        LICENCE:    MIT OPENSOURCE LICENCE                           ###
###                                                                                     ###
###                            CODE IS FREE TO USE AND MODIFY                           ###
###########################################################################################


# THIS FILE IS DEDICATED FOR THE ABOUT PAGE IN THE APPLICATION WHICH ACTS AS A OR AS A HELP TAB OF THE SOFTWARE APPLICATION
#BELOW IS A DUMM RANDOM GENERATED TEXT TO DISPLAY IN THE ABOUT HOME PAGE.
#KEEP ALL YOUR ABOUT IN ONE PLACE, ASSIGN IT TO A VARIABLE AND THEN IMPORT THIS FILE TO MAIN.PY FILE, AN DUSE THIS VARIABLE FOR THE DOCUMENT.
aboutHome ='Bu arayüz, kargo işlemlerini yönetmek için kullanıcı dostu bir tasarıma sahip olup, kargoların rota ve adres bilgilerini net bir şekilde gösterirken, aynı zamanda ilgili kişilerin iletişim bilgilerini düzenli bir biçimde sunar. Kullanıcılar, her kargonun gideceği adresi ve güzergahı rahatça görebilirken, her gönderi için alıcı ve gönderici bilgilerini hızlıca erişebilir. Kargo takip numarası, teslimat durumu gibi detaylar da kolayca görüntülenebilir. Arayüz, tüm bu bilgileri görsel olarak ayrıştırarak kullanıcının rahatça işlem yapmasını sağlar.'